...
- `narration-final.mp3` - optional private final track generated with the
  participant's ElevenLabs voice; preview and render scripts prefer it when
  present;
- `narration-final.wav` - reproducible offline fallback generated by
  `render_narration.ps1` and used automatically when the private MP3 is absent.
...
In a fresh clone, generate the offline narration before previewing or rendering:

```powershell
.\demo_assets\render_narration.ps1
```

The generated WAV and the optional private ElevenLabs MP3 are both ignored by
Git. Set `DEMO_NARRATION` to either filename to override automatic selection.
